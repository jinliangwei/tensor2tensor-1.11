...
class MtfModel(t2t_model.T2TModel):
  ...
  def estimator_model_fn(cls,
    ...
    tf.logging.info(hparams.mesh_shape)
    ...
    tf.logging.info(mesh_shape.to_string)
    ...
    if use_tpu:
      ...
      if data_parallelism is None or len(data_parallelism.ps_devices) == 1:
        ...
        tf.logging.info("not use tpu, mesh_shape is " +
                        " mesh_shape.size is " + str(mesh_shape.size)
                        + " num_ps_devices = " + str(data_parallelism.ps_devices))
